...
%%
%! Apply FFT to each columns of the matrix, then shifts the DC to the DFT centre. The value of omega_s in each row is defined by the formula omega_s = x * (2*pi / dx) where dx=1.
...
% @retval omega_s value of omega_s in each row
function [Fourier_Radon omega_s] = apply_fft1(Radon,DEBUG)

% Apply FFT to each column of the radon image
Fourier_Radon = fft(Radon);

% Label the omega_s,theta axes;
dx=1;
[size_omega_s size_theta] = size(Fourier_Radon);
omega_s = [0:(size_omega_s/2-1)  (-size_omega_s/2):-1] * 2*pi/dx;

% Shift the DC to the DFT centre
omega_s = fftshift(omega_s);
Fourier_Radon = fftshift(Fourier_Radon,1);

if(DEBUG)
THETA = 1:size_theta; %we do not need to know the angles in this function

save_image(THETA,omega_s, real(Fourier_Radon),...
	'Fourier transform of Radon Space, Real Part',...
	'theta_i','omega_s');	% Save the radon image (real part)
save_image(THETA,omega_s, imag(Fourier_Radon),...
	'Fourier transform of Radon Space, Imaginary Part',...
	'theta_i','omega_s');	% Save the radon image (imaginary part)

stem(omega_s, abs(Fourier_Radon(:,1)));
axis tight;
